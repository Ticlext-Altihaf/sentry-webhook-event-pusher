...
async fn handle_sentry(item: web::Json<sentry_definition::SentryEvent>) -> impl Responder {
    ...
    let res = send_to_email(item.clone()).await;
    match res {
...
mod tests {
    ...
    fn get_sample_event() -> Result<SentryEvent, &'static str> {
        let path = Path::new("examples.jsonl");
        
        let lines_res = read_lines(path).unwrap();
        let sample: SentryEvent;
        for line in lines_res {
            let event_res: SentryEvent = serde_json::from_str(&line.unwrap()).unwrap();
            sample = event_res;
            return Ok(sample);
        }
        Err("No sample event found")
    }

    #[test]
    fn send_email() {
        let sample = get_sample_event().unwrap();
        // make sure env set
        dotenv().ok();
        let res = executor::block_on(super::send_to_email(sample));
        assert_eq!(res, Ok(()));
    }
